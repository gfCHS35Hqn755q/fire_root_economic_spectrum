...
community_model <- lm(fire_effect_cover ~ average_colonisation, data = community_comp)
p_community_comp <- ggplot(community_comp, aes(x=average_colonisation, y=fire_effect_cover)) + 
  geom_point() + 
  stat_smooth(method = "lm") +
  geom_label(aes(x = 0, y = 15), hjust = 0, 
             label = paste("Adj R2 = ",signif(summary(community_model)$adj.r.squared, 5), 
                           "\nIntercept =",signif(community_model$coef[[1]],5 ), 
                           " \nSlope =",signif(community_model$coef[[2]], 5), 
                           " \np =",signif(summary(community_model)$coef[2, 4], 5),
                           "\nn = 6")) +
  ylab("Increase in percent cover / fire frequency") +
  xlab("Average AMF Colonisation") + 
  ggtitle(expression(paste("Relationship between increase in percent cover per fire frequency and average AMF colonisation"))) +
  theme_pubclean() +
  scale_colour_Publication()
